...
def load_workbook_grid(workbook_path, wanted_od):
    """ Load the ELISA data from selected OD channel."""
...
def generate_qc_summary(r2, results_df, cv_df, std_low, std_high, x, corrected_df):
    ...
    sample_rows = results_df[
        (results_df["Sample"].str.upper() !="STD")
        & (results_df["Sample"].str.upper() != "CONTROL")
        & (results_df["Sample"].str.upper() != "BLANK")
        ].copy()
...
def main():
    ...
    parser.add_argument(
        ...
        help = (
            ...
            "(e.g. 'OD(550)'. If omitted, the programme will ask for it.)"
        )
    )
    ...
    first_std_concentration = get_argument_or_input(
        args.std_start,
        "Enter the starting (highest) standard concentration in ng/mL (e.g. 32):\n> ",
        float
    )
